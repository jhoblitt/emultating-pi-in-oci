...
# Download the image
RUN wget https://repo.almalinux.org/rpi/9/images/AlmaLinux-9-RaspberryPi-latest.aarch64.raw.xz
ENV IMAGE_FILE=AlmaLinux-9-RaspberryPi-latest.aarch64.raw
...
RUN OFFSET=$(fdisk -lu ${IMAGE_FILE} | awk '/^Sector size/ {sector_size=$4} /FAT32 \(LBA\)/ {print $3 * sector_size}') && \
